...
class ModelTester:
    ...
    def test(self, model, dataset, num_votes=100):
        ...
        test_path = join('test') # Save all results in the same path for the convinience of 6fold evaluation
        makedirs(test_path) if not exists(test_path) else None  
        makedirs(join(test_path, 's3dis_preds')) if not exists(join(test_path, 's3dis_preds')) else None
                ...
                if last_min + 1 < new_min:
                    ...
                    if int(np.ceil(new_min)) % 1 == 0:
                        ...
                        for i_test in range(num_val):
                            ...
                            write_ply(join(test_path, 's3dis_preds', name), [preds, labels], ['pred', 'label'])
